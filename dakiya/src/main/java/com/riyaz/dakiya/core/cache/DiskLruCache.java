package com.riyaz.dakiya.core.cache;
...
public class DiskLruCache {
    ...
    private static String getFilePath(File cacheDir, String key) throws UnsupportedEncodingException {
        return cacheDir.getAbsolutePath() + File.separator +
                CACHE_FILENAME_PREFIX + URLEncoder.encode(key.replace("*", ""), "UTF-8");
    }
    public Boolean remove(@NotNull String key){
        try{
            return remove(mCacheDir, key);
        } catch (UnsupportedEncodingException e){
            return false;
        }
    }

    public static Boolean remove(File cacheDire, String key) throws UnsupportedEncodingException {
        String filePath = getFilePath(cacheDire, key);
